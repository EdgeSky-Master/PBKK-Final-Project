<x-layout>
    <x-slot:title>{{ $title }}</x-slot:title>
        ...
        {{-- <article class="py-8 max-w-screen-md border-b border-gray-500">
            ...
            <div >
                By
                 <a href="/authors/{{ $post->author->username }}" class="hover:underline text-base text-gray-500">{{ $post->author->name }}</a> in <a href="/categories/{{ $post->category->slug }}" class="hover:underline text-base text-gray-500">{{ $post->category->name }}</a>| {{ $post->created_at->format('j F Y') }} 
            ...
            <a href="/posts" class="font-medium text-blue-500 hover:underline">&laquo; Back to Previous page </a>
        </article> --}}
